	...
	$("#leagueButton").on("click",function(){
		$.ajax({
			...
			if(partTeam !=null||undefined){
				...
				'</h3><br><h5>Last Updated On : '+leagueResponse[i].lastUpdated+'</h5>'+
				...
				// $("#teamResponse[i].id").on("click",function myFunction(){
					...
					$.ajax({
						...
						for(var j = 0; j < teamResponse.teams.length; j++){
							...
							var teamTable ='<div class="brown-text"><div class="col s12"> <div class="card"><h4>' + 
							teamResponse.teams[j].name+'<hr><img id="images" src="'+
							teamResponse.teams[j].crestUrl+'">'+
							'</h4><div><a class="col-sm-12 col-md-6" id="teamStat" href="team_fixtures.html" onclick="teamFixtures('+teamResponse.teams[j]._links.fixtures+')"  class="waves-effect waves-light btn">'+"Team Fixtures"+'</a></div>'+
							' </div> </div></div>';
